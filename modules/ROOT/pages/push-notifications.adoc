// start {partialsdir}/attributes.adoc[]
include::_partials/attributes.adoc[]
// end {partialsdir}/attributes.adoc[]

:toclevels: 4

:toc:

// For more information, see: https://redhat-documentation.github.io/modular-docs/

:context: {unifiedpush-service}
ifdef::context[:parent-context: {context}]

[id='a-collection-of-modules']
= Using the {unifiedpush-service} Mobile Service

[[introduction]]
The {unifiedpush-service} service allows you to send native push notifications to different mobile operating systems.

* Code once and push notifications to iOS and Android.
* Push notifications to defined groups.
* Push notifications to either iOS only or Android only.
* Push notifications to different variants of a mobile app.
* Push notifications from different back-end apps to the same mobile app.

The {unifiedpush-service} service offers a unified Notification Service API to the above mentioned Push Network Services.
It can be seen as a _broker_ that distributes push messages to different 3rd party Push Networks.

Currently the service supports:

* link:https://developer.apple.com/notifications/[Apple Push Notification Service, window="_blank"]
* link:https://firebase.google.com/products/cloud-messaging/[Firebase Cloud Messaging, window="_blank"]

include::_partials/push/ref_terminology.adoc[leveloffset=+2]

WARNING: The payload of the push notification is delivered to 3rd party Push Network providers, Google or Apple. {org-name} recommends that users do not send any sensitive personal or confidential information (for example, a social security number, financial account or transactional information) as part of any Push Notification. Users of push notifications should not have an expectation of secure transmission.

For analytic purposes on our link:#_dashboard[Dashboard] we store the content of the +alert+ key sent to the UnifiedPush Server.
The content of the +alert+ key belongs to the metadata, which is deleted after 30 days, using a nightly job within the UnifiedPush Server.

NOTE: You are only able to send push notifications to a real device, sending push notifications to an emulator fails.

.Prerequisites

* [x] You are running OpenShift with {product-name} as described in xref:getting-started.adoc[Setting up AeroGear {product-name} on OpenShift].
* [x] You have provisioned Mobile Developer Console as described in xref:getting-started.adoc[Provisioning Mobile Developer Console].

[[setting-up-the-idm-service]]
== Setting Up the {unifiedpush-service} Mobile Service
include::_partials/push/1_obtaining_firebase_cloud_messaging_credentials.adoc[leveloffset=+2]
// already in intro
// include::_partials/push/1-1_push-intro.adoc[leveloffset=+2]
include::_partials/push/2_enable_push_notifications_for_ios_app.adoc[leveloffset=+2]
include::_partials/push/3_provisioning-the-push.adoc[leveloffset=+2]
include::_partials/push/4_binding-an-app-to-a-service.adoc[leveloffset=+2]
include::_partials/push/4_5_downloading-configuration-file.adoc[leveloffset=+2]
include::_partials/push/5_setting-up-the-push-sdk.adoc[leveloffset=+2]
include::_partials/push/6_configuring-the-service.adoc[leveloffset=+1]
include::_partials/push/7_sending-a-push-notification.adoc[leveloffset=+2]
include::_partials/push/8_handling-push-notifications.adoc[leveloffset=+2]

[[monitoring]]
// start metrics/monitoring-keycloak.adoc[leveloffset=1]
// include::_partials/metrics/monitoring-keycloak.adoc[leveloffset=2]

== Additional resources

* For more information about OpenID, see the link:https://openid.net/[OpenID Foundation, window="_blank"] website.
* See the link:{sso-docs-link}[{sso-name} documentation, window="_blank"] for more info.

// Restore the context to what it was before this assembly.
ifdef::parent-context[:context: {parent-context}]
ifndef::parent-context[:!context:]
