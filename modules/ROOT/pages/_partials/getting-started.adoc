...
{product-name} allows you to perform Cloud Native Mobile App Development using OpenShift as the back-end for your mobile apps. 
You create a representation of the mobile app that you are developing locally and bind your mobile app to mobile services such as {keycloak-service}, {unifiedpush-service}, {metrics-service} and others.
This makes many of the common tasks associated with mobile development much easier and quicker to implement.

A configuration file named `mobile-services.json` acts as the link between your {mobile-client} in OpenShift and your local app in development. 
This configuration file is used to initialize the {SDK} in your mobile app and to connect to the back-end {product-name} you have provisioned on OpenShift.


include::./mdc-introduction.adoc[tags={include-tags}]
...
* Register a {mobile-client} and bind it to a Mobile Service ({keycloak-service})
* Set up a local development environment
* Configure and run the showcase app 
// end::excludeUpstream[]